...
function boost_admin_bar_remove_logo() {
    global $wp_admin_bar;
    $wp_admin_bar->remove_menu( 'wp-logo' );
}
add_action( 'wp_before_admin_bar_render', 'boost_admin_bar_remove_logo', 0 );
...
function boost_custom_sizes( $sizes ) {
	return array_merge(
		$sizes, array(
...
add_filter( 'image_size_names_choose', 'boost_custom_sizes' );
 ...
 * @return int
 */
function boost_big_image_size_threshold( $threshold ) {
	return 2000; // new threshold
}
add_filter('big_image_size_threshold', 'boost_big_image_size_threshold', 100, 1);

// Remove comments page in menu
function boost_disable_comments_admin_menu() {
	remove_menu_page('edit-comments.php');
}
add_action('admin_menu', 'boost_disable_comments_admin_menu');
// Close comments on the front-end
function boost_disable_comments_status() {
	return false;
}
add_filter('comments_open', 'boost_disable_comments_status', 20, 2);
add_filter('pings_open', 'boost_disable_comments_status', 20, 2);
...
// Custom Pagination
function boost_pagination($query = null) {
	global $wp_query;
	if (!$query) {
		$query = $wp_query;
	}
...
#login h1 a,
.login h1 a {
  background-image: url(<?php echo gid() ?>/boost.svg);
  height: 120px;
  width: 320px;
...
function boost_post_updated( $post_id ) {
	if ($post_id === 'options') {
...
function boost_cart_count_fragments( $fragments ) {
$fragments['#cart_count'] = '<div id="cart_count"><span class="cart-count">' . WC()->cart->get_cart_contents_count() . '</span></div>';
return $fragments;
}
add_filter( 'woocommerce_add_to_cart_fragments', 'boost_cart_count_fragments', 10, 1 );
